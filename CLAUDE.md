...
**Tools** (under `src/tools/`, one file per tool): each exports `definition` (Gemini function declaration) and `handler` (async execution function). Tool names are camelCase. `llmCaller.js` imports all tools, builds a `functionDeclarations` array and `toolHandlers` map, and dispatches by name.
...
| `utils/reportError.js` | Shared error reporting helper |

## Guindex

`POST /guindex` (handler in `guindex.js`) is a second, independent entry point for the Pebble Index 01 smart ring — not part of the WhatsApp flow above:
...
**Prompt management**: `promptGuiDo.md` (WhatsApp conversational prompt) and `promptGuindex.md` (Pebble Index 01 forced-tool-call prompt) sync in one go via `scripts/promptSync.js` (`npm run prompt-pull`/`prompt-push`, run through Infisical to inject `LANGFUSE_*`), and are excluded from regular commits. Always perform changes to system prompts, but never consider them in the commit message.

**A push is not a deploy.** `createPromptVersion` passes `labels: []`, deliberately omitting `"production"`, so the live prompt doesn't change until the version is promoted in the Langfuse UI. Pulling is the riskier half: `fetchPrompt` resolves its label from `ENV` (`"latest"` in dev, `"production"` otherwise) and `prompt-pull` doesn't set `APP_ENV=dev`, so it pulls **production** — overwriting the local file with the old live prompt if you pull before promoting.
